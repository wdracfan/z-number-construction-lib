...
def normal_integral(a, b, c, d, mu, sigma):
...
def expon_integral(a, b, c, d, mu, l):
  def xint(x0, x1):
    x0 = max(x0, mu)
    x1 = max(x1, mu)
...
def uniform_integral(a, b, c, d, u, v):
  def xint(x0, x1):
    x0 = max(x0, u)
    x1 = min(x1, v)
    if x1 <= x0:
...
def calculate_integrals(a_part: FS, distributions: dict[str, list]):
  ...
  for key in distributions:
    if key == 'normal':
      integrals[key] = [normal_integral(a_part.a, a_part.b, a_part.c, a_part.d, distribution.stats()[0], np.sqrt(distribution.stats()[1])) for distribution in distributions[key]]
    elif key == 'exponential':
      integrals[key] = [expon_integral(a_part.a, a_part.b, a_part.c, a_part.d, distribution.stats()[0] - np.sqrt(distribution.stats()[1]), 1 / np.sqrt(distribution.stats()[1])) for distribution in distributions[key]]
    elif key == 'uniform':
      integrals[key] = [uniform_integral(a_part.a, a_part.b, a_part.c, a_part.d, (distribution.stats()[0] * 2 - np.sqrt(12 * distribution.stats()[1])) / 2, (distribution.stats()[0] * 2 + np.sqrt(12 * distribution.stats()[1])) / 2) for distribution in distributions[key]]
...
def calculate_distances(data: np.ndarray, distributions: dict[str, list], function: str = 'euclide') -> dict[str, np.ndarray]:
  ...
  for key in distributions:
    ...
    for distribution in tqdm(distributions[key]):
      if function == 'euclide':
        dists[key].append(euclide_distance(distribution.pdf, histogram, data.min(), data.max()))
      if function == 'manhattan':
        dists[key].append(manhattan_distance(distribution.pdf, histogram, data.min(), data.max()))
      if function == 'chebyshev':
        dists[key].append(chebyshev_distance(distribution.pdf, histogram, data.min(), data.max()))
    dists[key] = np.array(dists[key])
